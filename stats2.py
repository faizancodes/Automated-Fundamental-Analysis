...
# With this repository, you can generate data from each stock in the S&P500 or Mid Cap stocks with valuations over $2B. 
# If you want to generate data from the Mid Cap stocks, uncomment the next line and comment out the following line.
...
saveToFileName = 'C:\\Users\\faiza\\OneDrive\\Desktop\\Stock Ratings\\' + stockSet + 'StockRatings' + str(d).replace('/', '.') + '.csv'
...
def getStockData(sym):
    ...
    page = requests.get('https://finviz.com/quote.ashx?t=' + sym, headers=agent)
    ...
    rawSector = rawCompanyName[rawCompanyName.find('f=sec') : rawCompanyName.find('f=sec') + 70]
    sector = rawSector[rawSector.find('>') + 1 : rawSector.find('</a>')].replace(' ', '')
    ...
    if len(fixString(companyName)) > 2 and len(marketCap) > 2:
        ...
        fixString(sector), fixString(industry), convertToLetterGrade(getCategoryGrade(sector.lower(), valuationStats)), 
