...
def getAverage(sector, metric, lessThan):
    ...
    ninty = np.percentile(smoothedList, 90)

    if lessThan == True:
        ninty = np.percentile(smoothedList, 10)
    
    #print(sector + ' ' + metric + ' Avg:', metricAvg, metricStd)
    #print('\nMetrics Available:', [metric[1 : ] for metric in sectorData[0][7 : ]])
    
    return ninty, metricStd
...
def getGrade(mtrc, val, sector):
    ...
    if mtrc == 'Forward PE':
        lessThan = True
        start, change = getAverage(sector, mtrc, lessThan)
...
saveToFileName = 'C:\\Users\\faiza\\OneDrive\\Desktop\\StockData\\Stats2Outputs\\' + searchThrough + 'Stats2OutputUpdated.csv'
