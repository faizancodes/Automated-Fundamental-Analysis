...
def getInfo(sym):
    ...
    overall.append([sym, round(overallRating), fixString(companyName), marketCap, fixString(sector), fixString(industry), convertToLetterGrade(getCategoryGrade(valuationStats)), convertToLetterGrade(getCategoryGrade(profitabilityStats)), convertToLetterGrade(getCategoryGrade(growthStats)), convertToLetterGrade(getCategoryGrade(perfStats)), forwardPE, peg, priceSales, priceBook, priceFCF, profitMargin, operMargin, grossMargin, roe, roa, epsThisY, epsNextY, epsNext5Y, salesQQ, epsQQ, perfMonth, perfQuarter, perfHalfY, perfYTD, perfYear, sma20, sma50, sma200, dividend, instTrans, instOwn, insiderOwn, insiderTrans, targetPrice, prevClose, priceProjection, RSI, shortFloat, fifty2WeekHigh, fifty2WeekLow, earningsDate])
...
with open('C:\\Users\\faiza\\OneDrive\\Desktop\\StockData\\Stats2Outputs\\S&P500Stats2Output.csv', 'w') as f:

    f.write('Symbols,Rating,Valuation Grade, Profitability Grade, Growth Grade, Performance Grade, Market Cap, Sector, Industry, Forward PE, PEG, Price/Sales, Price/Book, Price/FCF, Net Margin, Operating Margin, Gross Margin, ROE, ROA, EPS This Y, EPS Next Y, EPS Next 5 Years, Sales Q/Q, EPS Q/Q, Perf Month, Perf Quarter, Perf Half Y, Perf YTD, Perf Year, SMA20, SMA50, SMA200, Dividend Yield, Inst Trans, Inst Own, Insider Own, Insider Trans, Target Price, Prev Close, Price Projection, RSI, Short Float, 52W High, 52W Low, Earnings Date' + '\n')

    for x in range(len(overall)):
        f.write(overall[x][0] + ',' + str(overall[x][1]) + ',' + overall[x][6] + ',' + overall[x][7] + ',' + overall[x][8] + ',' + overall[x][9] + ',' + overall[x][3] + ',' + overall[x][4] + ',' + overall[x][5] + ',' + str(overall[x][10]) + ',' + str(overall[x][11]) + ',' + str(overall[x][12]) + ',' + str(overall[x][13]) + ',' + str(overall[x][14]) + ',' + str(overall[x][15]) + ',' + str(overall[x][16]) + ',' + str(overall[x][17]) + ',' + str(overall[x][18]) + ',' + str(overall[x][19]) + ',' + str(overall[x][20]) + ',' + str(overall[x][21]) + ',' + str(overall[x][22]) + ',' + str(overall[x][23]) + ',' + str(overall[x][24]) + ',' + str(overall[x][25]) + ',' + str(overall[x][26]) + ',' + str(overall[x][27]) + ',' + str(overall[x][28]) + ',' + str(overall[x][29]) + ',' + str(overall[x][30]) + ',' + str(overall[x][31]) + ',' + str(overall[x][32]) + ',' + str(overall[x][33]) + ',' + str(overall[x][34]) + ',' + str(overall[x][35]) + ',' + str(overall[x][36]) + ',' + str(overall[x][37]) + ',' + str(overall[x][38]) + ',' + str(overall[x][39]) + ',' + str(overall[x][40]) + ',' + str(overall[x][41]) + ',' + str(overall[x][42]) + ',' + str(overall[x][43]) + ',' + str(overall[x][44]) + ',' + str(overall[x][45]) + '\n')
