...
with open('C:\\Users\\faiza\\OneDrive\\Desktop\\StockData\\StockSharpe\\Stats2Output.csv', 'w') as f:

    f.write('Symbols,Rating,Valuation Grade, Profitability Grade, Growth Grade, Performance Grade, Market Cap, Sector, Industry, Forward PE, PEG, Price/Sales, Price/Book, Price/FCF' + '\n')

    for x in range(len(overall)):
        f.write(overall[x][0] + ',' + str(overall[x][1]) + ',' + overall[x][6] + ',' + overall[x][7] + ',' + overall[x][8] + ',' + overall[x][9] + ',' + overall[x][3] + ',' + overall[x][4] + ',' + overall[x][5] + ',' + str(overall[x][10]) + ',' + str(overall[x][11]) + ',' + str(overall[x][12]) + ',' + str(overall[x][13]) + ',' + str(overall[x][14]) + '\n')
